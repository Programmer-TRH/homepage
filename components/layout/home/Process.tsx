...
export default function Process() {
  // Section + each step reveal hooks
  const { ref, show } = useReveal();
  const stepRefs = [useReveal(), useReveal(), useReveal()];

  const steps = [
    {
      icon: Process1,
      title: "Contact Us",
      desc: "Get in touch via phone, online form or visit our store",
    },
    {
      icon: Process2,
      title: "Get Valuation",
      desc: "Our experts evaluate your gold at current market rates",
    },
    {
      icon: Process3,
      title: "Accept Offer",
      desc: "Review our competitive offer and accept if satisfied",
    },
  ];

  return (
    <section ref={ref} className="bg-[#100f14] py-16 md:py-24">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        {/* Section Title */}
        <h2
          ref={ref}
          className={`text-3xl md:text-4xl font-bold text-white text-center mb-4 reveal-base reveal-up ${
            show ? "reveal-show" : ""
          }`}
        >
          How To Sell Your Gold
        ...
        <p
          ref={ref}
          className={`text-gray-400 text-center mb-16 reveal-base reveal-up ${
            show ? "reveal-show" : ""
          }`}
          ...
          {steps.map((step, i) => {
            ...
            return (
              <div
                key={i}
                ref={stepRef.ref}
                className={`bg-[#1b1a1f] rounded-lg p-8 flex flex-col items-center text-center relative reveal-base reveal-up ${
                  stepRef.show ? "reveal-show" : ""
                }`}
                style={{ transitionDelay: `${0.1 + i * 0.1}s` }} // staggered
              >
                <div className="w-20 h-20 bg-[#fbbf24] rounded-full flex items-center justify-center mb-6">
                  <Image
                    src={step.icon}
