...
export default function Testimonials() {
  ...
  const testimonialRefs = [useReveal(), useReveal(), useReveal()];
  ...
  return (
    <section ref={ref} className="bg-[#0f1419bf] py-16 md:py-24 relative">
      <div className="absolute inset-0 -z-10">
        <Image
          src={BuildingImage}
          alt="Background building image"
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {testimonials.map((testimonial, i) => {
            const tRef = testimonialRefs[i];
            return (
              <div
                key={i}
                ref={tRef.ref}
                className={`bg-[#1a1f2e9f] rounded-lg p-6 border border-[#2d3748] reveal-base reveal-up ${
                  tRef.show ? "reveal-show" : ""
                }`}
                style={{ transitionDelay: `${0.1 + i * 0.1}s` }} // stagger effect
              >
                <div className="flex items-center gap-4 mb-4">
                  <div
                    className={`w-12 h-12 rounded-full ${testimonial.color} flex items-center justify-center`}
